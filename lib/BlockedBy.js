 ...
 */
async function blockedBySelf({ waitingKey, buildId, jobId }) {
...
class BlockedBy extends NodeResque.Plugin {
   ...
   */
    ...
    async beforePerform() {
        ...
        if (blockedBy.length > 0) {
            ...
            await Promise.all(blockedBy.map(async (key) => {
                ...
                const regex = new RegExp(`${runningJobsPrefix}(\\d*)`);

                if (val !== null) {
                    // const [, jid] = key.match(/mockRunningJobsPrefix_(\d*)/);

                    const [, jid] = regex.exec(key);
     ...
     * @param  {Array}   blockingJobsRunning  List of job Ids that are blocking this current build
     * @return {Promise}
     */
