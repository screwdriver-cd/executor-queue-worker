 ...
 * @param  {String}      runningBuildId Running build Id
 * @return {Boolean}                    Whether this build is blocked
 */
async function blockedBySelf({ waitingKey, buildId, runningBuildId }) {
    ...
    if (waitingBuilds.length > 0) {
        ...
        if (firstWaitingBuild !== buildId) {
            await this.reEnqueue(waitingKey, buildId, runningBuildId);
